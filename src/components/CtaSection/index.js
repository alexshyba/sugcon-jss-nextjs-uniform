  ...
  <div className="w-full md:w-1/3 p-6 flex flex-col flex-grow flex-shrink">
    <div className="flex-1 bg-white rounded-t rounded-b-none overflow-hidden shadow">
      <a href="#" className="flex flex-wrap no-underline hover:no-underline">
        <Text tag="div" field={title} className="w-full font-bold text-xl text-gray-800 px-6" />
        <RichText tag="p" className="text-gray-800 text-base px-6 mb-5" field={text} />
      </a>
    </div>
  ...
  return (
    <section className="bg-white border-b py-8">
      <div className="container mx-auto flex flex-wrap pt-4 pb-12">
        ...
        {teasers && teasers.map((teaser, index) => <Teaser key={index} {...teaser.fields} />)}
      </div>
    </section>
  );
};
